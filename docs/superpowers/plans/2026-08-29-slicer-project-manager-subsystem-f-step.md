...
- [ ] `packages/desktop/src/previews.ts`: `PREVIEW_MAX_STEP_BYTES = DEFAULT_MAX_STEP_BYTES` beside
      ...
      for one.
- [ ] **Three false docblocks this task falsifies that no spec section lists** (constraint 14). Two
      sit on the line this task edits, which is how they were missed: a sweep for the claim found
      them, the list did not. The third sits twenty lines above one of them and was found by task
      1's review sweep rather than by any list at all — it is already false at task 1's tip, where
      the formula it states is the one `queue.ts:77` now labels "Before STEP support", so between
      that commit and this one two live docblocks over one formula disagree about whether it is
      current. Task 1 deliberately did not touch it, because `env.ts` is this task's file.
...
| `server/src/env.ts:137`, `resolvePreviewConcurrency`   | Carries **both** sentences this subsystem corrects elsewhere, verbatim: "This and `SPM_MAX_MESH_MB` are the two halves of one number: a worker's peak is one mesh plus a fixed reader window, so the queue's peak is roughly `concurrency × (mesh + 80 MB) + 120 MB`". The first is the server's twin of `desktop/src/previews.ts`'s "the two numbers are one budget", which task 1 corrected; the second is the exact formula `queue.ts:77` now labels "Before STEP support". Must say the STEP floor is a third term, is per process and does **not** multiply by concurrency, and must point at the same corrected table `queue.ts` and the README carry rather than restating a formula for the fourth time |
